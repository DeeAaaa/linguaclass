...
function DashboardPage({ user, setCurrentPage }) {
  ...
  const [activeSubject, setActiveSubject] = useState(null); // 'english' | 'math' | 'science' | null
  const [expandedChildId, setExpandedChildId] = useState(null); // for My Children expand
            ...
            {myKids.length > 0 ? (
              <div className="my-children-cards-grid">
                {myKids.map(kid => {
                  ...
                  const isExpanded = expandedChildId === kid.id;
                  return (
                    <div key={kid.id} className={`my-child-dash-card${isExpanded ? ' expanded' : ''}`}>
                      <div className="child-dash-header" onClick={() => setExpandedChildId(isExpanded ? null : kid.id)} style={{ cursor: 'pointer' }}>
                        ...
                        <button className="child-expand-btn" onClick={e => { e.stopPropagation(); setExpandedChildId(isExpanded ? null : kid.id); }}>
                          {isExpanded ? '▲' : '▼'}
                        </button>
                      </div>
                      ...
                      {isExpanded && (
                        <div className="child-expand-panel">
                          <div className="child-detail-row">
                            <Icons.User />
                            <span className="child-detail-label">Teacher:</span>
                            <span className="child-detail-value">{kid.teacher}</span>
                          </div>
                          {kid.scheduleDay && (
                            <div className="child-detail-row">
                              <Icons.Calendar />
                              <span className="child-detail-label">Schedule:</span>
                              <span className="child-detail-value">{kid.scheduleDay}, {kid.scheduleTime}</span>
                            </div>
                          )}
                          <div className="child-detail-row">
                            <Icons.Mail />
                            <span className="child-detail-label">Parent:</span>
                            <span className="child-detail-value">{kid.parentName} ({kid.parentEmail})</span>
                          </div>
                          <div className="child-expand-actions">
                            <button className="child-action-btn edit" onClick={() => setCurrentPage?.('studentrecords')}>
                              <Icons.Edit /> Edit
                            </button>
                            <button className="child-action-btn view" onClick={() => setCurrentPage?.('studentrecords')}>
                              <Icons.StudentRecords /> View Records
                            </button>
                          </div>
                        </div>
                      )}
                      <div className="child-dash-footer">
                        ...
                        {kid.scheduleDay && <span className="child-schedule">{kid.scheduleDay}</span>}
                      </div>
                    </div>
                  );
                })}
      ...
      <section className="subjects-section">
        ...
        <div className="subjects-grid">
          {MAIN_SUBJECTS.map(subject => (
            <div
              key={subject.id}
              className={`subject-card${activeSubject === subject.id ? ' active' : ''}`}
        ...
        {activeSubject === 'english' && (
          <div className="subject-content-panel">
            <div className="subject-content-header">
              <span className="subject-content-title">🎬 Learn English with Videos</span>
              <button className="subject-content-close" onClick={() => setActiveSubject(null)}><span>✕</span></button>
            </div>
...
function App() {
  ...
  const renderPage = () => {
    ...
    // Family users ONLY: dashboard, calendar, files, video room
    if (isFamily && !['dashboard', 'calendar', 'files', 'video', 'contacts'].includes(currentPage)) {
